...
class SpotifyWebAPI
{
    ...
    private function setupMock($fixture = 200)
    {
        if (is_int($fixture)) {
            ...
            $fixture = $_SERVER['DOCUMENT_ROOT'].'/ricardo/investigacion/spotify/spotify-web-api-php-master/tests/fixtures/' . $fixture . '.json';
        ...
        $request = $this->getMock('SpotifyWebAPI\Request.php');
        $request->method('api')
                ->willReturn($return);

        $api = $this->SpotifyWebAPI($request);
        return $api;
    }
    ...
    {
        $api = $this->setupMock(200);
        $response = $api->unfollowPlaylist('mcgurk', $this->playlistID);
